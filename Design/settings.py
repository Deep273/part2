...
SECRET_KEY = 'django-insecure-luwo_*zgcr_qu-+q#cre$d)t-6c0envcxopg3ufd=hjdh9%!1e'
...
TEMPLATES = [
    {
        ...
        'DIRS': [
            BASE_DIR / 'templates',  # Эта строка может быть необязательной, если шаблоны находятся внутри приложения
        ],
        'APP_DIRS': True,
        'OPTIONS': {
            'context_processors': [
...
STATICFILES_DIRS = [
    BASE_DIR / 'static',  # Путь для статических файлов в корне проекта (если они находятся там)
]
...
ASGI_APPLICATION = 'Design.asgi.application'